...
class ActorSDG:
    ...
    async def run(self):
        ...
        try:
            ...
            seed_out = os.path.join(base_out, f"seed_{int(seed_val)}_numseed_{int(char_num_rand)}_3")
    ...
    async def _record_trajectories(self, out_dir: str):
        ...
        for prim in char_prims:
            ...
            w = csv.writer(f)
            w.writerow(["time_s", "frame", "prim_path", "tx", "ty", "tz", "qw`", "qx", "qy", "qz", "roll", "pitch", "yaw"])
            writers[pstr] = w
            files[pstr] = f
    ...
    import omni.replicator.core as rep
    def _add_topview_camera_and_writer(
        self,
        cam_path: str = "/World/TopViewCamera",
        out_subdir: str = "topview_rgb",
        height_offset: float = 2.0,     # meters above the highest object
        resolution: tuple = (1920, 1080)
    ):
        """
        Create a top-down camera centered over /World and write RGB frames with Replicator.
        """
        # Get stage and /World prim
        stage = omni.usd.get_context().get_stage()
        world_prim = stage.GetPrimAtPath("/World")
        if not world_prim or not world_prim.IsValid():
            print("[TopView] /World not found; skipping topview camera setup.")
            return

        # Compute world-space AABB of /World to find center and top
        bbox_cache = UsdGeom.BBoxCache(0.0, includedPurposes=[UsdGeom.Tokens.default_, UsdGeom.Tokens.render])
        world_bbox = bbox_cache.ComputeWorldBound(UsdGeom.Imageable(world_prim))
        aabb = world_bbox.ComputeAlignedBox()
        center = aabb.GetCenter()
        max_pt = aabb.GetMax()

        # Place camera above the scene, looking straight down (-Z)
        cam_z = float(max_pt[2] + height_offset)
        cam_pos = Gf.Vec3d(float(center[0]), float(center[1]), cam_z)
        ...
        camera = _UsdGeom.Camera(cam_prim)

        # Set transform: translation = (cx, cy, zTop+offset), rotation = look straight down
        # In OpenUSD, a rotation that points -Z with +Y up can be set via a quaternion or xform ops.
        # We'll set an explicit transform with XformOps: rotateX(90) then rotateZ(0) then translate.
        xform = _UsdGeom.Xformable(cam_prim)
        # Clear existing ops (optional safety)
        for op in xform.GetOrderedXformOps():
            xform.RemoveXformOp(op)

        # Rotate so that camera looks along -Z:
        # A simple way: rotateX(-90 deg) to make camera aim down if default looks along -Y.
        # Depending on your stage up-axis and camera default, this may be rotateX(90) or (-90).
        # Try -90 first; if image is sideways, switch to +90.
        xform.AddRotateXOp().Set(-90.0)
        xform.AddTranslateOp().Set(cam_pos)

        # Set a reasonable focal length / aperture (optional)
        camera.CreateFocalLengthAttr(24.0)
        camera.CreateHorizontalApertureAttr(20.955)  # OV default for 1920 px
        camera.CreateVerticalApertureAttr(15.290)

        # Create a render product for this camera at desired resolution
        rp = rep.create.render_product(str(cam_path), resolution)

        # Initialize a BasicWriter to dump RGB images for the top view
        out_dir = os.path.join(self.output_path or os.getcwd(), out_subdir)
        os.makedirs(out_dir, exist_ok=True)
        writer = rep.WriterRegistry.get("BasicWriter")
        writer.initialize(output_dir=out_dir, rgb=True)  # add other streams if needed
        writer.attach([rp])

        print(f"[TopView] Camera at {tuple(cam_pos)} writing RGB to: {out_dir}")
