...
def _ensure_debug_dirs(out_dir: str):
    dbg        = os.path.join(out_dir, "debug_np")
    pos_dir    = os.path.join(dbg, "position")
    orn_dir    = os.path.join(dbg, "orientation")
    robot_dir  = os.path.join(dbg, "robot")
    rel_dir    = os.path.join(dbg, "relative")

    for d in (dbg, pos_dir, orn_dir, robot_dir, rel_dir):
        os.makedirs(d, exist_ok=True)

    return dbg, pos_dir, orn_dir, robot_dir, rel_dir


def save_pos_orn_np(out_dir: str, frame: int, pos_np: np.ndarray, orn_np: np.ndarray):
    """Save per-frame position/orientation into separate subfolders."""
    dbg, pos_dir, orn_dir, _, _ = _ensure_debug_dirs(out_dir)
    np.save(os.path.join(pos_dir, f"position_{frame:06d}.npy"), pos_np)
    np.save(os.path.join(orn_dir, f"orientation_{frame:06d}.npy"), orn_np)
    return dbg  # so we can reuse the root dir


def save_robot_np(out_dir: str, frame: int, robot_pos: np.ndarray):
    """Save per-frame robot absolute position."""
    dbg, _, _, robot_dir, _ = _ensure_debug_dirs(out_dir)
    np.save(os.path.join(robot_dir, f"robot_pos_{frame:06d}.npy"), robot_pos)


def save_rel_np(out_dir: str, frame: int, rel_pos: np.ndarray):
    """Save per-frame position relative to robot."""
    dbg, _, _, _, rel_dir = _ensure_debug_dirs(out_dir)
    np.save(os.path.join(rel_dir, f"relative_position_{frame:06d}.npy"), rel_pos)
def find_index_for_robot(target_paths, key: str = "Nova_Carter") -> int:
    key_l = key.lower()
    for i, p in enumerate(target_paths):
        if p.lower() == key_l: return i
    for i, p in enumerate(target_paths):
        if p.split("/")[-1].lower() == key_l: return i
    for i, p in enumerate(target_paths):
        if key_l in p.lower(): return i
    raise KeyError(f"Robot '{key}' not found in target paths.")

def get_robot_and_relative_positions(pos_np: np.ndarray, robot_idx: int):
    robot_pos = pos_np[robot_idx]
    rel = pos_np - robot_pos
    return robot_pos, rel
...
class ActorSDG:
    ...
    async def run(self):
        ...
        try:
            ...
            params["writer"] = "SceneGrapghWriter"
            params["parameters"] = {
        # cadence
                "caption_interval": 1,          # << every frame
                "scene_graph_interval": 1,      # build graph each frame (needed for captions)
                "skip_frames": 0,
                "writer_interval": 1,
    ...
    # IMPORTANT: caption_config controls exactly what gets produced
                "caption_config": {
        # keep it caption-only; don’t dump scene-graph JSON or viz
                "caption_only": True,
                "save_full_scene_graph": False,
                "save_pruned_scene_graph": False,
                "visualize_caption": False,

        # VLM behaviors
                "global_caption": False,
                "qa_caption": False,
                "brief_caption": True,      # << brief text caption

        # must be True or captions will be skipped in code path
                "export_edges": True,
                },
            }
            writer_selection.content_prop.set_value(params)
            base_out = params.get("output_dir") or os.getcwd()
            seed_out = os.path.join(base_out, f"seed_{int(seed_val)}_numseed_{int(char_num_rand)}_np")
    ...
    def _enable_extensions(self):
        ...
        ext_manager.set_extension_enabled_immediate("isaacsim.replicator.caption.core", True)
    ...
    async def _record_trajectories(self, out_dir: str):
        ...
        for prim in char_prims:
            ...
            w = csv.writer(f)
            w.writerow(["time_s", "frame", "prim_path", "tx", "ty", "tz", "roll", "pitch", "yaw"])
            writers[pstr] = w
            files[pstr] = f
        ...
        while getattr(self, "_traj_running", False) and not self._sim_app.is_exiting():
            ...
            # [NEW 1/3] save raw arrays
            save_pos_orn_np(out_dir, frame, pos_np, orn_np)

            try:
                nova_idx = find_index_for_robot(target_paths, key="Nova_Carter")
                robot_pos, rel_pos = get_robot_and_relative_positions(pos_np, nova_idx)
            # optional: persist per-frame robot & relative positions
                save_robot_np(out_dir, frame, robot_pos)
                save_rel_np(out_dir, frame, rel_pos)
            ...
            for prim in targets:
                ...
                #qw, qx, qy, qz = [float(v) for v in orn_np[i]]
