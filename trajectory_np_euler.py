...
def world_to_robot_transform(rel: np.ndarray, robot_yaw: float) -> np.ndarray:
    xy = rel[:,:2]
    R = np.array([
...
class ActorSDG:
    ...
    async def run(self):
        ...
        try:
            ...
            seed_out = os.path.join(base_out, f"seed_{int(seed_val)}_numseed_{int(char_num_rand)}_camera")
    ...
    async def _record_trajectories(self, out_dir: str):
        ...
        while getattr(self, "_traj_running", False) and not self._sim_app.is_exiting():
            ...
            try:
                ...
                nova_yaw = orn_np[nova_idx,3]
                rel_pos = world_to_robot_transform(rel_pos, robot_yaw=nova_yaw)
            # optional: persist per-frame robot & relative positions
                robot_info = np.hstack((robot_pos, np.array([nova_yaw])))
                save_robot_np(out_dir, frame, robot_info)
            ...
            for prim in targets:
                ...
                eulers = quats_to_euler_angles(orn_np, degrees=False, extrinsic=True)
