...
class ActorSDG:
    ...
    async def run(self):
        ...
        try:
            ...
            seed_out = os.path.join(base_out, f"seed_{int(seed_val)}_numseed_{int(char_num_rand)}_2")
    ...
    # -------- helpers --------
        ...
        def get_manager_name(prim):
            # Prefer names used by the anim people/robot systems; fall back to full prim path
            try:
                from isaacsim.replicator.agent.core.stage_util import CharacterUtil
                n = CharacterUtil.get_character_name(prim)
                if n:
                    return n
            except Exception:
                pass
            try:
                ...
                n = RobotUtil.get_robot_name(prim)
                if n:
                    return n
            except Exception:
                pass
        ...
        try:
            ...
            if robots_root and robots_root.IsValid():
                robot_prims = list(robots_root.GetChildren())
    ...
    # writers/files keyed by FULL prim path; separate map for manager-name lookup
        writers, files, prim_to_mgrname = {}, {}, {}

        for prim in char_prims:
            pstr = prim.GetPath().pathString
            prim_to_mgrname[pstr] = get_manager_name(prim)
            csv_path = os.path.join(char_dir, f"{path_to_filename(pstr)}_trajectory.csv")
            f = open(csv_path, "w", newline="", encoding="utf-8")
            w = csv.writer(f)
            w.writerow(["time_s", "frame", "prim_path", "tx", "ty", "tz"])  # position only
            writers[pstr] = w
            files[pstr] = f
        ...
        for prim in robot_prims:
            pstr = prim.GetPath().pathString
            prim_to_mgrname[pstr] = get_manager_name(prim)
            csv_path = os.path.join(robot_dir, f"{path_to_filename(pstr)}_trajectory.csv")
            f = open(csv_path, "w", newline="", encoding="utf-8")
            w = csv.writer(f)
            w.writerow(["time_s", "frame", "prim_path", "tx", "ty", "tz"])  # position only
            writers[pstr] = w
        ...
        names = []
        try: 
            names = mgr.get_all_managed_characters()
        except Exception:
            names = []
        print("Manager knows:", names)
        ...
        targets = list(dict.fromkeys(list(char_prims) + list(robot_prims)))
        ...
        while getattr(self, "_traj_running", False) and not self._sim_app.is_exiting():
            ...
            _  = Usd.TimeCode(t * tps)  # kept for consistency; not used further

            for prim in targets:
                if not prim.IsValid():
                    continue
                pstr = prim.GetPath().pathString
                name = prim_to_mgrname[pstr]

                # Position from GlobalCharacterPositionManager only
                try:
                    cur = mgr.get_character_current_pos(name)
                except Exception:
                    cur = None

                if cur is None:
                    continue  # if manager doesn't have it, skip logging this frame

                writers[pstr].writerow([
                    f"{t:.6f}", frame, pstr,
                    f"{float(cur.x):.6f}", f"{float(cur.y):.6f}", f"{float(cur.z):.6f}"
                ])
